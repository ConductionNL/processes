# procestypecatalogus chart

The Helm chart installs procestypecatalogus and by default the following dependencies using subcharts:
...
```bash
helm repo add procestypecatalogus https://raw.githubusercontent.com/ConductionNL/procestypecatalogus/master/api/helm/
helm repo update
```
...
```bash
helm install my-procestypecatalogus ConductionNL/procestypecatalogus --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `procestypecatalogus` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `procestypecatalogus` |
